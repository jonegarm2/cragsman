...
function search(req, res){
    searchApiData(req.params.searchterm, function(err, response, body) {
        if (err) return res.status(401).json(err);
        res.status(200).json(JSON.parse(body)); 
    })
}
...
function searchApiData(searchterm, cb) {
    // http://api.sierratradingpost.com/api/1.0/products/search~{search-term-url-encoded-with-hyphens}/?api_key=YourApiKey
    request(`${API}products/search~${searchterm}/?api_key=${process.env.API_KEY}`, function(error, response, body) {
        cb(error, response, body)
    })
}
